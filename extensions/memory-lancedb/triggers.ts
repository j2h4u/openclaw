...
const PREFERENCE_TRIGGERS: TriggerPattern[] = [
  // English
  {
    pattern: /\bi (like|love|prefer|enjoy|hate|dislike|can'?t stand)\b/i,
    category: "preference",
    lang: "en",
  },
...
const FACT_TRIGGERS: TriggerPattern[] = [
  ...
  {
    pattern: /я (была?|жила?|бывала?|ездила?|учил(?:ся|ась)|родил(?:ся|ась))/i,
    category: "fact",
    lang: "ru",
  },
...
function getFilteredTriggers(languages: LanguageFilter): TriggerPattern[] {
  ...
  const langSet = new Set<LanguageCode>(Array.isArray(languages) ? languages : [languages]);
 ...
 */
export function getAllMatches(text: string, languages: LanguageFilter = "auto"): TriggerPattern[] {
  const triggers = getFilteredTriggers(languages);
